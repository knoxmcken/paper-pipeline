...
def test_download_rejects_html_under_http_200(tmp_path):
    ...
    with pytest.raises(fetch.FetchError) as exc:
        fetch.download_pdf("2401.1", tmp_path, session=session, url="https://x/y.pdf",
                           delay=0, attempts=1)
    assert "did not return a PDF" in str(exc.value)
    assert not list(tmp_path.glob("*.pdf"))


def test_download_refuses_to_invent_a_url(tmp_path):
    """A DOI-keyed paper with no OA link must not be fetched from a made-up address."""
    session = FakeSession([])
    with pytest.raises(fetch.FetchError) as exc:
        fetch.download_pdf("doi:10.1109/cloud-summit.2025.00030", tmp_path, session=session)
...
def test_download_writes_pdf_atomically(tmp_path):
    ...
    info = fetch.download_pdf("2401.1", tmp_path, session=session, url="https://x/y.pdf", delay=0)
    assert info["bytes"] == 13 and info["skipped"] is False
    assert (tmp_path / "2401.1.pdf").read_bytes().startswith(b"%PDF-")
    assert not list(tmp_path.glob("*.part"))


def test_download_skips_existing_file_without_needing_a_url(tmp_path):
